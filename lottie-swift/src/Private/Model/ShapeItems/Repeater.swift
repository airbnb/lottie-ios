...
final class Repeater: ShapeItem {
  ...
  required init(from decoder: Decoder) throws {
    ...
    self.rotationX = try transformContainer.decodeIfPresent(KeyframeGroup<Vector1D>.self, forKey: .rotationX) ?? KeyframeGroup(Vector1D(0))
    self.rotationY = try transformContainer.decodeIfPresent(KeyframeGroup<Vector1D>.self, forKey: .rotationY) ?? KeyframeGroup(Vector1D(0))
