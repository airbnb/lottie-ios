...
+ (instancetype)animationNamed:(NSString *)animationName inBundle:(NSBundle *)bundle {
    ...
    NSString *key = [NSString stringWithFormat:@"%@%@", animationName, bundle.bundlePath];
    LOTComposition *comp = [[LOTAnimationCache sharedCache] animationForKey:key];
    if (comp) {
    ...
    if (JSONObject && !error) {
        ...
        [[LOTAnimationCache sharedCache] addAnimation:laScene forKey:key];
