...
+ (instancetype)animationNamed:(NSString *)animationName {
  ...
  if (JSONObject && !error) {
    LOTComposition *lotScene = [[LOTComposition alloc] initWithJSON:JSONObject];
    [[LOTAnimationCache sharedCache] addAnimation:lotScene forKey:animationName];
    return [[LOTAnimationView alloc] initWithModel:lotScene];
  }
...
+ (instancetype)animationFromJSON:(NSDictionary *)animationJSON {
  LOTComposition *lotScene = [[LOTComposition alloc] initWithJSON:animationJSON];
  return [[LOTAnimationView alloc] initWithModel:lotScene];
}
...
- (instancetype)initWithContentsOfURL:(NSURL *)url {
  ...
  if (self) {
    LOTComposition *lotScene = [[LOTAnimationCache sharedCache] animationForKey:url.absoluteString];
    if (lotScene) {
      [self _initializeAnimationContainer];
      [self _setupWithSceneModel:lotScene restoreAnimationState:NO];
    } else {
      ...
      dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
        ...
        dispatch_async(dispatch_get_main_queue(), ^(void){
          [[LOTAnimationCache sharedCache] addAnimation:lotScene forKey:url.absoluteString];
          [self _initializeAnimationContainer];
          [self _setupWithSceneModel:lotScene restoreAnimationState:YES];
        });
      });
    }
  }
