...
+ (id)shapeItemWithJSON:(NSDictionary *)itemJSON frameRate:(NSNumber *)frameRate compBounds:(CGRect)compBounds {
  ...
  if ([type isEqualToString:@"gr"]) {
    ...
    LOTShapeGradientFill *gradientFill = [[LOTShapeGradientFill alloc] initWithJSON:itemJSON frameRate:frameRate];
    return gradientFill;
    ...
    LOTShapeRepeater *repeater = [[LOTShapeRepeater alloc] initWithJSON:itemJSON frameRate:frameRate];
    return repeater;
  } else {
    NSString *name = itemJSON[@"nm"];
