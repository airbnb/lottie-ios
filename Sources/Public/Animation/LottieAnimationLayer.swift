...
open class LottieAnimationLayer: CALayer {
    ...
    open func playDelayTime(_ beginTime:CFTimeInterval? = nil,customDuration:CFTimeInterval? = nil,superLayer:CALayer? = nil, completion: LottieCompletionBlock? = nil) {
        guard let animation else { return }
        ...
        let context = AnimationContext(
            ...
            closure: completion)
        removeCurrentAnimationIfNecessary()
        addNewAnimationForContext(context, beginTime: beginTime,superLayer:superLayer,customDuration: customDuration)
    }
    ...
    fileprivate func addNewAnimationForContext(_ animationContext: AnimationContext, beginTime:CFTimeInterval? = nil,superLayer:CALayer? = nil, customDuration:CFTimeInterval? = nil) {
        ...
        if let newDuration = customDuration {
            layerAnimation.duration = newDuration
        } else {
        ...
        if let begTime = beginTime,let layer = superLayer {
            layerAnimation.isAdditive = false
            layerAnimation.autoreverses = false
            layerAnimation.fillMode = .forwards
            layerAnimation.isRemovedOnCompletion = true
//            layerAnimation.beginTime = begTime
            let currentLayerTime = convertTime(begTime, from: layer)
            
            print("==currentLayerTime===>\(currentLayerTime)")
            let beginTime = currentLayerTime - (timeOffset * 1 / Double(abs(animationSpeed)))
            print("==beginTime===>\(currentLayerTime)")
...
//            layer.add(layerAnimation, forKey: activeAnimationName)
//        } else {
//
//        }
